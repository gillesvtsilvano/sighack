<?
	...
	$sql = "select codigo_turma, num_turma, horario, ch, nome_disc from (disciplina join aluno_turma) natural join turma where codigo_disc=codigo_turma && matricula_aluno='".$_SESSION['usuario']."'";
	...
	if( $contagem > 0 ){
		...
		while ( $linha = mysql_fetch_array( $confirmacao ) ) {
			...
			echo("<td align=\"center\" ><a href=\"concluir_desmatricula.php?cod_disc=".$linha[0]."\">desmatricular</a></td>" );
			echo("</tr>");
		}
		echo("</table>");
	}
