<?
	...
	if( $contagem > 0 ){
		...
		echo("<td align=\"center\" width=\"61%\"> <b>Nome </b></td>" );
		...
		while ( $linha = mysql_fetch_array( $confirmacao ) ) {
			$sql = "select * from turma, aluno_turma where codigo_disc='".$linha[0]."' || codigo_turma='".$linha[0]."'";
			...
			if( mysql_num_rows($resultado) == 0 ){
				echo("<td align=\"center\" > <a href=\"remocao.php?tipo_remocao=3&cod_disc=" . $linha[0] . "\"> remover </a></td>" );
			}
			else{
		...
		echo("Nao ha disciplinas cadastradas");
	}
?>
